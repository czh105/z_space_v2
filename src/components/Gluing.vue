		...
		setup(props, {
			...
			function allDebuggerAnimate() {
				...
				if (topicNum.value === 8.5) {
					if(!isDebugger.value){
						actionHistory.pushModelsActionHistory([j61, tj], actionEnum.VISIBLE);
					}
				...
				}else if (topicNum.value === 10.5) { // 放胶枪工具的调试运行
					if(!isDebugger.value){
						actionHistory.pushModelsActionHistory([j61, tj], actionEnum.VISIBLE);
					}
					isDebugger.value = true;
...
<template>
	<div class="controls_box">
		...
		<div class="left_control" v-if="!showHelp" >
