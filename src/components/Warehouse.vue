		...
		setup(props, {
			...
			function allDebuggerAnimate() {
				...
				if (topicNum.value === 10.5) {
					let obj = player.scene.getObjectByName(`cang1`);
					if(!isDebugger.value){
						actionHistory.pushModelsActionHistory([obj], actionEnum.POSITION);
						actionHistory.pushModelsActionHistory([cangitem1, cangitemtou], actionEnum.VISIBLE);
					}
	...
	<div class="controls_box">
		...
		<div class="left_box" v-if="showlr" v-show="!isFold">
          ...
          " class="action_btn1" @click="switchAction()"></div>
				<div class="t1-1 conBtn" v-if="topicNum === 3" @click="setLeftContent('仓库单元调试实训任务'), topicNext(true)">
					下一步
				</div>
				<div class="t1-1 conBtn" v-if="topicNum === 5" @click="setLeftContent('仓库单元调试实训任务'), topicNext(true)">
					下一步
				</div>
          ...
          " class="action_btn_box">
			...
			<!-- 仓库单元调试实训任务 -->
			<div class="robot_opt" :style="topicOneImg" v-if="leftContent === '仓库单元调试实训任务'">
				...
				<div class="codepage" v-if="topicNum === 7">
					<div class="conBtn" style="width: 153px; height: 14px; left: 95px; top: 102px">
						<thinking-select :list="[
                { label: 'TOpStoragecylinder1', value: 0 },
                { label: 'ToTDigGrip', value: 1 },
              ]" @changeData="changeData"></thinking-select>
					</div>
					<div class="conBtn" style="width: 157px; height: 14px; left: 91px; top: 237px">
						<thinking-select :list="[
				...
				<div class="codepage" v-if="topicNum === 8">
					<div class="conBtn" style="width: 148px; height: 14px; left: 123px; top: 145px">
						<thinking-select :list="[
                { label: 'TOpStoragecylinder1', value: 0 },
                { label: 'ToTDigGrip', value: 1 },
              ]" @changeData="changeData"></thinking-select>
					</div>
					<div class="conBtn" style="width: 148px; height: 14px; left: 124px; top: 189px">
						<thinking-select :list="[
                { label: 'TOpStoragecylinder1', value: 1 },
                { label: 'ToTDigGrip', value: 0 },
              ]" @changeData="changeData"></thinking-select>
